...
- **누적 막대는 두 입력 모양을 다 받는다.** 숫자 컬럼을 여럿 골라 컬럼 하나를 층 하나로 쓰거나(wide — 엑셀에서 표를 통째로 잡는 방식), `누적 기준`에 범주 컬럼을 골라 값으로 층을 가르거나(long). **둘을 함께 쓰지 말 것** — 시리즈가 컬럼 N개 × 범주 M개로 늘어 색 슬롯 8개를 넘는다. 서로를 잠그고 왜 잠겼는지 트리거에 적는다(`Y축(우)`↔`분할`과 같은 방식). 쌓이는 순서는 고른 순서가 아니라 **파일의 컬럼 순서**다 — 누른 차례로 두면 같은 컬럼을 골라도 두 카드가 다르게 보인다(절대 원칙 1).
...
- **`spec.mapping`은 슬롯마다 컬럼 이름 하나, `value`만 이름 배열도 된다** — 누적 막대에서
  숫자 컬럼 하나하나가 층이 되는 wide 모양이다(엑셀·Power BI가 하는 방식). 문자열 하나는
  길이 1인 목록과 같은 뜻이라 예전 대시보드가 그대로 열린다. **모양을 읽을 때는
  `pickedColumns()`를 쓸 것** — `typeof === "string"`을 곳곳에서 다시 물으면 한 군데만
  빠뜨려도 배열이 문자열처럼 취급되어 조용히 틀린다.
