...
- **이중 축(y축 2개)은 선 차트에서만, 조건을 지켜서.** 두 축의 눈금을 어떻게 맞추느냐에 따라 선이 교차하는 자리가 달라져서 데이터에 없는 상관관계가 보인다. 그래서 기본은 여전히 차트 2개나 공통 기준 지수화이고, 한 화면에서 봐야 할 때만 `Y축(우)` 슬롯을 쓴다. 지킬 것: **두 축 모두 0에서 시작**, 범례에 (좌)/(우) 표기, 축 이름 옆에 그 선의 색 마크, 표 보기 제공. 막대·영역·산점도에는 주지 않는다.
...
- 컬럼 타입(숫자/날짜/범주) 추론은 실패할 수 있다. 추론 결과를 사용자가 고칠 수 있게 만들 것 — 조용히 잘못 추론하는 것이 최악이다. 구현은 `src/shared/lib/infer-types.ts`, UI는 사이드바 "컬럼" 섹션.
