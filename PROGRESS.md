...
**UI 셸 + CSV/TSV 파싱 + 컬럼 타입 추론까지 완료.** 다음은 슬롯별 후보 필터링, 그다음 차트.

파일을 열면 인코딩을 감지해 파싱하고, 컬럼 타입을 추론해 사이드바에 보여준다.
추론이 틀리면 사용자가 고칠 수 있다.
아직 없는 것: 슬롯별 후보 필터링·자동 선택, 차트 렌더링.
...
| `src/lib/infer-types.ts` | 컬럼 타입 추론 (숫자/날짜/범주 + 신뢰도) |
...
`ColumnInfo`는 `type`(현재) / `inferred`(추론 원본) / `confidence` / `sampled`를 들고 있다.
`type !== inferred`면 사용자가 손댄 것이고, 그때는 경고를 더 띄우지 않는다.
