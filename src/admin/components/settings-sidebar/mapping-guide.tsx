...
export function MappingGuide({
  ...
  return (
    <>
      ...
      {/*
        쌓을 것을 정하는 길이 둘이라 어느 쪽을 쓸지가 데이터 모양에 달렸다. 그
        판단이 이 슬롯에서 제일 먼저 막히는 자리다 — 층이 컬럼으로 펼쳐진 표에서
        누적 기준에 넣을 컬럼을 찾다 보면 아무것도 못 고른다.
      */}
      {chartType === "stacked" && (
        <p className="mt-2.5 border-t border-background/20 pt-2.5 text-background/70">
          층이 컬럼으로 나뉘어 있으면(<code>OFF</code> · <code>ON</code> 컬럼) 값에서 여럿
          고릅니다. 한 컬럼 안에 층 이름이 값으로 들어 있으면 누적 기준으로 가릅니다. 둘은
          함께 쓸 수 없습니다.
        </p>
      )}
    </>
